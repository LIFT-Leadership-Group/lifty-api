...
export const LinkedinCampaignRequest = z.discriminatedUnion("operation", [
  z.object({ operation: z.literal("prepare"), payload: z.object({ workspace: LinkedinWorkspace, lead_id: ref, connection_ref: ref, text, campaign_ref: ref.optional() }).strict() }).strict(),
...
export const LinkedinCampaignResult = z.object({
  ...
  content: z.object({
    invitation: z.object({ note: z.null() }).strict(), message: z.object({ text }).strict(),
...
export function linkedinCampaignResultFor(input: LinkedinCampaignInput, data: unknown): LinkedinCampaignOutput {
  ...
  if (("campaign_ref" in input.payload && input.payload.campaign_ref && input.payload.campaign_ref !== result.campaign_ref)
    ...
    || (input.operation === "prepare" && (input.payload.lead_id !== result.lead_id || input.payload.connection_ref !== result.connection_ref || input.payload.text !== result.content.message.text)))
