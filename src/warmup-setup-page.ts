...
export function renderWarmupSetupPage(record:WarmupSetupRecord, intent:string, csrf:string):string {
  ...
  return shell("Set up email warmup", `<div class="eyebrow">Email warmup</div><h1>Warm up your mailbox.</h1>
<p>Sign in with Google once. Lifty handles the warmup settings and tells you when campaigns can send from this mailbox.</p>
<div class="mailbox"><span class="hint">Your mailbox, verified through Unipile</span><strong>${escape(record.email)}</strong><span class="hint">Sign in to Google with this exact address. Gmail and Google Workspace only.</span></div>
...
<fieldset><legend>Name on warmup emails</legend><div class="grid">
...
<div class="notice"><p><strong>Google may say it hasn't verified Lifty yet.</strong> That's expected during the pilot. Choose <strong>Advanced</strong>, then <strong>Go to Lifty</strong>.</p></div>
<p class="hint">Google asks for full mailbox access because Mailivery, our warmup provider, sends warmup emails from this mailbox and replies to the ones it receives. Lifty passes that access to Mailivery once and does not store it. Warmup emails don't count toward your daily outreach limit.</p>
<div class="actions"><button type="submit">Continue with Google</button><p class="hint">This starts warmup, not outreach. A new outreach mailbox needs 21 active, healthy warmup days before campaigns can send from it.</p></div></form>
